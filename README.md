...
- **Автоматические повторные попытки** — yt-dlp: 3 попытки на запрос, 5 на фрагмент; при HTTP 403/410/429 или гео-блокировке — повтор с браузерными заголовками (User-Agent, Accept-Language, geo-bypass)
...
- **Проверка на активную трансляцию** — ссылка на незавершённый live-эфир отклоняется до начала загрузки
